...
const EditHeader = () => {
    ...
    const [headerData, setHeaderData] = useState<HeaderItem | null>(null);
    ...
    useEffect(() => {
        const fetchHeaderData = async () => {
            try {
                ...
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/header/${id}`, {
                    headers: {
                ...
                const data: HeaderItem = await response.json();
                setHeaderData(data);
                setHeaderName(data.header_name);

                if (data.images) {
                    setPreviewImage(`${process.env.NEXT_PUBLIC_API_URL}/${data.images}`);
                }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        formData.append('section_id', headerData?.section_id || '');

        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/header/${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
            ...
            <div className="flex">
                ...
                <div className="flex-1 p-6 lg:p-10 ml-0 lg:ml-62">
                    ...
                    <div className="mt-8 mx-auto">
                        ...
                        >
                            <h2 className="text-2xl font-bold mb-6 text-gray-800">Edit Header: {headerData.header_name}</h2>
                            ...
                            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 mt-2">
                                        Current image: {headerData.images}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="bg-gray-50 p-4 rounded">
                                    ...
                                    <p><span className="font-medium">ID:</span> {headerData.id}</p>
                                    <p><span className="font-medium">Section ID:</span> {headerData.section_id}</p>
                                    <p><span className="font-medium">Header name:</span> {headerData.header_name}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4">
                                <button
                                    type="button"
                                    onClick={() => router.push('/admin/header')}
