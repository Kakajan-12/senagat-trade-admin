...
const Mail = () => {
    ...
    const [selectedAddress, setSelectedAddress] = useState<Mail | null>(null);
    ...
    const handleDeleteClick = (address: Mail) => {
        ...
        setModalMessage(`Are you sure you want to delete this mail "${address.mail}"? This action cannot be undone.`);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/mail/${selectedAddress.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });

            setMails(prev => prev.filter(addr => addr.id !== selectedAddress.id));
                    ...
                    <div className="flex-1 p-6 lg:p-10 ml-0 lg:ml-62">
                        ...
                        <div className="mt-8 bg-white rounded-lg shadow-lg p-6">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                                <h2 className="text-2xl font-bold text-gray-800">Mails</h2>
                                <Link
                                    href="/admin/mail/add-mail"
                            ...
                            <div className="overflow-x-auto rounded-lg border border-gray-200">
                                <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                                    ...
                                    {mails.length === 0 ? (
                                        ...
                                        mails.map((mail) => (
                                            <tr key={mail.id} className="divide-x-1 divide-gray-200">
                                                <td className="py-4 px-4">
                                                    <span className="text-sm text-gray-600">#{mail.id}</span>
                                                </td>
                                                <td className="py-4 px-4">
                                                    <div className="text-sm text-gray-800">{mail.mail}</div>
                                                </td>
                                                <td className="py-4 px-4">
                                                    <div className="flex space-x-2">
                                                        <Link
                                                            href={`/admin/mail/edit-mail/${mail.id}`}
