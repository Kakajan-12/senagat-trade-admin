...
const Phone = () => {
    ...
    const [selectedAddress, setSelectedAddress] = useState<Phone | null>(null);
    ...
    useEffect(() => {
        const fetchAddresses = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/phone`, {
                    headers: {
    ...
    const handleDeleteClick = (address: Phone) => {
        setSelectedAddress(address);
        setModalMessage(`Are you sure you want to delete this phone "${address.phone}"? This action cannot be undone.`);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/phone/${selectedAddress.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });

            setPhones(prev => prev.filter(addr => addr.id !== selectedAddress.id));
                    ...
                    <div className="flex-1 p-6 lg:p-10 ml-0 lg:ml-62">
                        ...
                        <div className="mt-8 bg-white rounded-lg shadow-lg p-6">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                                <h2 className="text-2xl font-bold text-gray-800">Phones</h2>
                                <Link
                            ...
                            <div className="overflow-x-auto rounded-lg border border-gray-200">
                                <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                                    ...
                                    {phones.length === 0 ? (
                                        ...
                                        phones.map((phone) => (
                                            <tr key={phone.id} className="divide-x-1 divide-gray-200">
                                                <td className="py-4 px-4">
                                                    <span className="text-sm text-gray-600">#{phone.id}</span>
                                                </td>
                                                <td className="py-4 px-4">
                                                    <div className="text-sm text-gray-800">{phone.phone}</div>
                                                </td>
                                                <td className="py-4 px-4">
                                                    <div className="flex space-x-2">
                                                        <Link
                                                            href={`/admin/phone/edit-phone/${phone.id}`}
